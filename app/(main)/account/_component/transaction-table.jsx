  ...
  <TableBody>
    {filteredAndSortedTransactions.length===0?(
      ...
      <TableCell> <Checkbox onCheckedChange={()=>handleSelect(transaction.id)}
        checked={selectedIds.includes(transaction.id)}/></TableCell>
      ...
      <TableCell className="text-right font-medium" style={{
        ...
        <TableCell>{transaction.isRecurring?(<TooltipProvider>
  <Tooltip>
...
):(<Badge variant="outline" className="gap-1"><Clock className='h-3 w-3'/>One-time</Badge>)}</TableCell>
<TableCell>
<DropdownMenu>
